...
<div class="modal" style="padding:15px;width:88%;max-width:380px;opacity:1;">
    ...
    <div class="content" style="max-height:420px;padding:15px 0 0 0;overflow-y:auto;">
        <div style="margin-bottom:10px;">
            <label style="display:flex;align-items:center;justify-content:space-between;font-size:.8rem;">
                <span>✨ 总开关</span>
                <label class="switch"><input type="checkbox" id="beautyToggle" checked onchange="toggleBeauty()"><span class="slider"></span></label>
            </label>
        </div>
        <div style="margin-bottom:10px;">
            <label style="display:flex;align-items:center;justify-content:space-between;font-size:.8rem;">
                <span>🔘 圆角卡片</span>
                <label class="switch"><input type="checkbox" id="beautyRadius" checked onchange="applyBeauty()"><span class="slider"></span></label>
            </label>
        </div>
        <div style="margin-bottom:10px;">
            <label style="display:flex;align-items:center;justify-content:space-between;font-size:.8rem;">
                <span>🌫️ 阴影效果</span>
                <label class="switch"><input type="checkbox" id="beautyShadow" checked onchange="applyBeauty()"><span class="slider"></span></label>
            </label>
        </div>
        <div style="margin-bottom:10px;">
            <label style="display:flex;align-items:center;justify-content:space-between;font-size:.8rem;">
                <span>💊 胶囊按钮</span>
                <label class="switch"><input type="checkbox" id="beautyPill" checked onchange="applyBeauty()"><span class="slider"></span></label>
            </label>
        </div>
        <div style="margin-bottom:10px;">
            <label style="display:flex;align-items:center;justify-content:space-between;font-size:.8rem;">
                <span>📐 紧凑布局</span>
                <label class="switch"><input type="checkbox" id="beautyCompact" onchange="applyBeauty()"><span class="slider"></span></label>
            </label>
        </div>
        <div style="margin-bottom:10px;">
            <label style="display:flex;align-items:center;justify-content:space-between;font-size:.8rem;">
                <span>🎯 标题渐变</span>
                <label class="switch"><input type="checkbox" id="beautyTitle" checked onchange="applyBeauty()"><span class="slider"></span></label>
            </label>
        </div>
        <div style="margin-bottom:10px;">
            <label style="display:flex;align-items:center;justify-content:space-between;font-size:.8rem;">
                <span>🌈 渐变背景</span>
                <label class="switch"><input type="checkbox" id="beautyGradient" onchange="applyBeauty()"><span class="slider"></span></label>
            </label>
        </div>
        <div style="margin-bottom:10px;">
            <label style="display:flex;align-items:center;justify-content:space-between;font-size:.8rem;">
                <span>🔍 输入框美化</span>
                <label class="switch"><input type="checkbox" id="beautyInput" checked onchange="applyBeauty()"><span class="slider"></span></label>
            </label>
        </div>
        <div style="margin-bottom:10px;">
            <label style="display:flex;align-items:center;justify-content:space-between;font-size:.8rem;">
                <span>🚫 隐藏插件按钮</span>
                <label class="switch"><input type="checkbox" id="beautyHidePlugins" onchange="applyBeauty()"><span class="slider"></span></label>
            </label>
        </div>
    ...
    const getCSS = () => {
        ...
        if (radius) {
            css += `.kano_function_main,.box,.deviceList li,.deviceList strong,.deviceList .card-item,.modal,.mask .modal,.status-container,.band-lock-container,.freq-lock-container,.func_list_container,.devices-mon{border-radius:${radiusVal}px!important;overflow:hidden;}\n`;
        }

        if (shadow) {
            css += `.kano_function_main,.box,.deviceList li,.status-container,.band-lock-container,.freq-lock-container,.func_list_container,.devices-mon{box-shadow:0 ${s}px ${s*2}px rgba(0,0,0,${(s*0.04).toFixed(2)})!important;transition:box-shadow .3s ease,transform .3s ease;}\n`;
        ...
        if (pill) {
            css += `button,.btn,.actions-buttons button,.box button{border-radius:50px!important;transition:all .2s ease!important;}\n`;
            css += `button:hover,.btn:hover{transform:scale(1.03);filter:brightness(1.15);}\n`;
        }

        if (compact) {
            css += `.container{max-width:640px!important;}\n.kano_function_main{margin-bottom:6px!important;}\n`;
        }

        if (title) {
            css += `.title{background:linear-gradient(90deg,#667eea,#764ba2);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;font-weight:700!important;}\n`;
        }

        if (gradient) {
            css += `body{background:linear-gradient(135deg,#0f0c29,#1a1a2e,#16213e)!important;}\n`;
        }

        if (input) {
            css += `input[type="text"],input[type="password"],input[type="number"],input[type="email"],textarea,select{border-radius:${Math.round(radiusVal/2)}px!important;border:1.5px solid rgba(255,255,255,0.15)!important;transition:border-color .3s,box-shadow .3s!important;}\n`;
        ...
        if (hidePlugins) {
            css += `.actions-buttons button:not(#beautyBtn){display:none!important;}\n`;
        }
    ...
    // 添加到 .actions-buttons
    const actionsContainer = document.querySelector('.actions-buttons');
    if (actionsContainer && !actionsContainer.querySelector('#beautyBtn')) {
